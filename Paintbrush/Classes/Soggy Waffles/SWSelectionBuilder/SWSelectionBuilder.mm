...
- (id) initWithBitmapImageRep:(NSBitmapImageRep *)imageRep point:(NSPoint)point tolerance:(CGFloat)tolerance
{
	...
	if ( self != nil ) {
		...
		mMaskData = (unsigned char*)calloc(mHeight, mMaskRowBytes);
		memset(mMaskData, 0xFF, mHeight * mMaskRowBytes);
		
		// Calloc marks them all as not visited
		mVisited = (BOOL *)calloc(mHeight * mWidth, sizeof(BOOL));
		
		// Calloc this guy too, just for fun
		mSegments = (SWSegment*)calloc(mHeight * mWidth, sizeof(SWSegment));
		mSegCt = 0;
        ...
        auto to_int = [](NSInteger input) -> int { return [[NSNumber numberWithInteger:input] intValue]; };
        
		int bitsPerSample = to_int([mImageRep bitsPerPixel]) / to_int([mImageRep samplesPerPixel]);
...
//		NSDictionary* segment = [[[mStack lastObject] retain] autorelease];
		SWSegment *segment = (SWSegment*)[[mStack lastObject] pointerValue];
...
- (unsigned int) pixelDifference:(NSPoint)point
{
	...
	int samplesPerPixel = to_int([mImageRep samplesPerPixel]);
	int i = 0;
	for (i = 0; i < (samplesPerPixel - 1); ++i) {
		//		if (mPickedPixel[i] != pixel[i]) {
		//			return -1;
		//		}
        unsigned int difference = (unsigned int)labs((long)mPickedPixel[i] - (long)pixel[i]);
