...
- (id)initWithSize:(NSSize)sizeIn
{
	...
	if (self)
	{
		...
		NSBitmapImageRep *tempMain = nil;
		NSBitmapImageRep *tempBuffer = nil;
		[SWImageTools initImageRep:&tempMain withSize:size];
		[SWImageTools initImageRep:&tempBuffer withSize:size];
		mainImage = tempMain;
		bufferImage = tempBuffer;
...
- (void)restoreBufferImageFromData:(NSData *)tiffData
{
	...
	if (!NSEqualRects(bufferImageRect, finalRect))
	{
		...
		NSBitmapImageRep *newBuffer = nil;
		[SWImageTools initImageRep:&newBuffer withSize:finalRect.size];
		bufferImage = newBuffer;
	}
