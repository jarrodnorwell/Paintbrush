...
- (void)windowDidLoad
{
	...
	[undoStepper setIntValue:[[[NSUserDefaults standardUserDefaults] valueForKey:kSWUndoKey] intValue]];
	[undoTextField setIntValue:[[[NSUserDefaults standardUserDefaults] valueForKey:kSWUndoKey] intValue]];
...
- (IBAction)selectPrefPane:(id)sender
{
	...
	NSView *view;
	NSString *title;
	[self viewForTag:to_int(tag) view:&view title:&title];
	[[self window] setTitle:title];

	NSView *previousView;
	[self viewForTag:currentViewTag view:&previousView title:&title];
	currentViewTag = to_int(tag);
